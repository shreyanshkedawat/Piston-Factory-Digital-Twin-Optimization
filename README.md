...
This repository contains a **Simulation-based Digital Twin** developed to simulate and optimize a piston manufacturing assembly cell. [cite_start]By leveraging **Discrete Event Simulation (DES)** and custom **FlexScript (C-based)** logic, this project identifies operational bottlenecks and prescribes optimized workflows to maximize system efficiency [cite: 1684-1685, 2040-2041].
...
* [cite_start]**Resource Optimization:** Boosted average machine utilization to **~93%** (up from baseline averages of ~35%), effectively eliminating idle-time waste .
...
* [cite_start]**Infrastructure:** 5 Processing Machines, 1 Loading Station, Input/Output Conveyors[cite: 1728].
* [cite_start]**Process Time:** 68 seconds per piston[cite: 1732].
* [cite_start]**Operator Specs:** Capacity of 2 units, max walking speed of 1.33 m/s, loading time of 6s, unloading time of 4s [cite: 1730-1734].

## 🧠 Optimization Strategy
[cite_start]The default factory logic resulted in severe bottlenecks, with machines sitting idle for up to 50% of the production cycle due to inefficient operator routing[cite: 1776]. I approached this problem in three phases:
...
[cite_start]*Global lists and process flows were used to dynamically assign tasks based on machine availability[cite: 1818, 2050].*

### 2. Algorithmic Pathfinding (FlexScript)
To eliminate wasted travel time, I wrote custom C-based **FlexScript**. [cite_start]The script generates all possible permutations of operator travel paths between available machines, calculating the mathematical minimum distance before executing the routing command [cite: 1910-1911].
...
### 3. Resource Scaling
[cite_start]After optimizing the programmatic logic, I scaled the workforce parameter from 1 operator to 2, dividing the tasks symmetrically to ensure continuous machine feeding and eliminate the final system bottleneck [cite: 1779, 2060-2061].
...
* [cite_start]**Resource Utilization:** Average machine capacity utilization skyrocketed from ~35% to **~93%**, effectively eliminating the system bottleneck .
